...
//  ST7789 INIT
// ==========================================================================
static const uint8_t PROGMEM init_operations[] = {BEGIN_WRITE,
                                                  WRITE_COMMAND_8,
                                                  0x11,
                                                  END_WRITE,
                                                  DELAY,
...
void setActiveProfile(uint8_t idx) {
  ...
  webLog("Profile: %s spd=%lu sg=%u", profiles[idx].name, (unsigned long)ui_speed_hz, RUN_SG_TRIP);
}
...
static void on_go_main(lv_event_t *e) {
  LV_UNUSED(e);
  go(main_scr);
}
...
void buildUI() {
  ...
  profile_scr = lv_obj_create(NULL);
  ...
  // Endpoint screens
  ep_up_scr = lv_obj_create(NULL);
  build_endpoint_screen(ep_up_scr, "EP UP", true);
  ep_dn_scr = lv_obj_create(NULL);
