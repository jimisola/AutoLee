...
typedef struct {
  ...
  uint32_t sample_points;  // Sample points used for deceleration phase. Note: |end_freq_hz -
                           // start_freq_hz| >= sample_points
 ...
 *      - ESP_OK if creating encoder successfully
 */
esp_err_t rmt_new_stepper_motor_uniform_encoder(
    const stepper_motor_uniform_encoder_config_t *config, rmt_encoder_handle_t *ret_encoder);
