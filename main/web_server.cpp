...
static std::string buildStateJSON() {
    ...
    std::string wfSSID = wifi_mgr::isConnected() ? "" // STA SSID isn't tracked separately here
                          : (wifi_mgr::isApMode() ? DEFAULT_AP_SSID
                                                   : "-");
    std::string wfIP = (wifi_mgr::isConnected() || wifi_mgr::isApMode()) ? wifi_mgr::ipAddress() : "-";
...
static std::string wifiConfigPage() {
    ...
    html += "<label>Or type SSID manually</label><input name='ssid_manual' placeholder='SSID "
            "(optional)'>";
    html += "<label>Password</label><input name='pass' type='password' placeholder='WiFi "
            "password'>";
    html += "<button class='btnSave' type='submit'>Save &amp; Reboot</button></form>";
    html += "<form method='POST' action='/clear'><button class='btnClear' "
...
void setupWebServer() {
    ...
    server.on("/save", HTTP_POST, [](PsychicRequest *req, PsychicResponse *res) {
        ...
        res->setCode(200);
        res->setContentType("text/html");
        res->setContent("<html><body style='font-family:sans-serif;text-align:center;padding:40px;"
    ...
    if (wifi_mgr::isApMode()) {
        static const char *probes[] = {"/generate_204",       "/gen_204",
