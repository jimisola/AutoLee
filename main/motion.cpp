...
static void showJamScreen() { ESP_LOGW(TAG, "JAM screen (UI not yet wired)"); }
static void setRunButtonState(bool running) { ESP_LOGI(TAG, "run button -> %d (UI not yet wired)", running); }
...
void recomputeEffectiveEndpoints() {
    autolee::Endpoints e = autolee::computeEffectiveEndpoints(
        endpointsCalibrated, rawUp, rawDown, upOffsetSteps, downOffsetSteps, OFFSET_MIN,
        OFFSET_MAX, ENDPOINT_GUARD);
    ...
    switch (runState) {
        case RUNNING: {
            ...
            if ((millis() - lastSGPrintMs) > 500) {
                int32_t distToTarget = labs(pos - currentTarget);
                ESP_LOGI(TAG, "RUN SG=%u trip=%u pos=%ld dist=%ld t=%lu hi=%u/%u", sg, RUN_SG_TRIP,
                         pos, (long)distToTarget, (unsigned long)sinceChange, runSGHighCount,
                         RUN_SG_HIGH_NEEDED);
            ...
            if (sg > RUN_SG_TRIP) {
                ...
                ESP_LOGI(TAG, "SG HIGH=%u trip=%u cnt=%u pos=%ld t=%lu", sg, RUN_SG_TRIP,
                         runSGHighCount, pos, (unsigned long)sinceChange);
