...
// Same formula TMCStepper uses for TMC5130/5160-family chips: CS = 32*sqrt2*I/1000*(Rsense+0.02)/Vfs - 1,
// picking the low-sensitivity full-scale voltage (0.180V, vsense=1) when the high-sensitivity
// range (0.325V) would clip the 5-bit CS field below its usable floor.
void rms_current(uint16_t mA) {
    ...
    tmc5160_fieldWrite(kIcID, TMC5160_IHOLD_FIELD, (uint32_t)lroundf(cs)); // hold = run (no idle current cut)
}

void en_pwm_mode(bool enabled) {
    tmc5160_fieldWrite(kIcID, TMC5160_EN_PWM_MODE_FIELD, enabled ? 1 : 0);
}
void TPWMTHRS(uint32_t threshold) { tmc5160_fieldWrite(kIcID, TMC5160_TPWMTHRS_FIELD, threshold); }
void TCOOLTHRS(uint32_t threshold) { tmc5160_fieldWrite(kIcID, TMC5160_TCOOLTHRS_FIELD, threshold); }
void semin(uint8_t value) { tmc5160_fieldWrite(kIcID, TMC5160_SEMIN_FIELD, value); }
void semax(uint8_t value) { tmc5160_fieldWrite(kIcID, TMC5160_SEMAX_FIELD, value); }
void seup(uint8_t value) { tmc5160_fieldWrite(kIcID, TMC5160_SEUP_FIELD, value); }
void sedn(uint8_t value) { tmc5160_fieldWrite(kIcID, TMC5160_SEDN_FIELD, value); }
void sgt(int8_t value) { tmc5160_fieldWrite(kIcID, TMC5160_SGT_FIELD, (uint32_t)(int32_t)value); }

uint32_t DRV_STATUS() { return (uint32_t)tmc5160_readRegister(kIcID, TMC5160_DRV_STATUS); }
uint16_t SG_RESULT() { return (uint16_t)tmc5160_fieldRead(kIcID, TMC5160_SG_RESULT_FIELD); }
