...
- **Status:** Superseded — the PlatformIO + pioarduino decision below was shipped as PR #3, then
  **not merged**: once the "Future direction — ESP‑IDF" research below showed the full port was
  more tractable than expected, the project went straight to native ESP‑IDF instead of taking the
  intermediate step. This ADR is kept for its research (the governance context, the verified
  per‑library ESP‑IDF support table, the `sdkconfig` calibration) — see
  [`docs/PLAN.md`](../PLAN.md) for the current, active migration.
- **Date:** 2026-07-18
...
| FastAccelStepper (motion) | ❌ **not pure ESP‑IDF** | *(Corrected — verified 2026‑07‑18 by reading the source, not just the "ESP‑IDF build matrix" claim.)* Its ESP‑IDF component mode genuinely requires **`arduino-esp32` running as an ESP‑IDF component** (its own README: "You must have Arduino available as a component"). Not usable without reintroducing Arduino. **Replaced by a native RMT‑based step generator** (`main/stepper.*`, adapted from Espressif's official `examples/peripherals/rmt/stepper_motor`). A TMC5160‑internal‑ramp‑generator alternative (no STEP/DIR pins at all) was considered and rejected — it needs a hardware pin strap (`SD_MODE`/`SPI_MODE`) that can't be confirmed for the board in use and most TMC5160 modules ship strapped for external STEP/DIR. | **Med** (was wrongly rated Low) |
...
| **ESP Async WebServer + Async TCP + SSE** | ❌ **not pure ESP‑IDF** | *(Corrected — verified 2026‑07‑18 by reading the actual README/registry page, not inferring "on the ESP Registry" ⇒ ESP‑IDF‑native. Same class of mistake as FastAccelStepper below.)* Its own README lists supported frameworks as **"Arduino Core 2.x and 3.x, PlatformIO"** only — no ESP‑IDF. The registry listing exists because the Component Registry also hosts Arduino‑framework libraries, not because it's pure‑IDF. **Replaced by native `esp_http_server`** (built into ESP‑IDF) + a hand‑rolled SSE handler (hold the `/api/v1/events` connection open, `httpd_resp_send_chunk()` on each broadcast) + `esp_ota`/`esp_https_ota` for the upload endpoint. *(This is the research-time recommendation; what actually shipped is [PsychicHttp](https://github.com/hoeken/PsychicHttp) on top of `esp_http_server` rather than a hand-rolled SSE handler - see `docs/PLAN.md` Phase 5 for why.)* | **Med** (was wrongly rated Low) |
...
**Effort summary (corrected): Display, Motion, and the web layer are Med; everything else Low; nothing High.**
