...
| WiFi + captive portal | ✅ component (recommended) | **Recommended: [MycilaESPConnect](https://github.com/mathieucarbou/MycilaESPConnect)** (MIT, actively maintained) — built *on the ESPAsyncWebServer we're already keeping*, so it drops in as the captive‑portal + provisioning manager with ~no new surface. Alternative, if preserving AutoLee's exact current WiFi UX (integrated scan/reset page) matters: port `wifi_ota.cpp`'s flow onto `esp_wifi` + `nvs` + the official ESP‑IDF `captive_portal` DNS example | **Low** (component) / **Low–Med** (roll‑your‑own) |
