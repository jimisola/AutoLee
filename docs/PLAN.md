...
- [x] **OTA watchdog crash fixed** - found via a real failed OTA upload + core dump decode: `pump_task`'s `broadcastState()` sends an SSE update every loop iteration, and that socket `send()` can block for the full default 5s timeout when a large OTA POST is also in flight on the same HTTP server (client busy uploading, not reading the SSE stream, so the TCP send buffer backs up). That's long enough to starve the IDLE task and trip the task watchdog - crashing the device mid-upload every time (`Panic reason: Task watchdog got triggered ... pump (CPU 0)`, confirmed via `idf.py coredump-info`'s thread backtrace showing `pump` blocked inside `PsychicEventSource::send`). Fixed narrowly: `broadcastState()` now skips SSE sends while an OTA upload is in progress. This is a targeted fix for the reproduced failure, not a general guarantee that SSE sends can never block `pump_task` (which also drives `handleMotion()`'s jam/homing dispatch) under other slow/lossy-network conditions - flagged for a future architectural fix (move `broadcastState()` off the safety-relevant task loop).
- **DoD:** ✅ **verified end-to-end on real hardware**, from a phone genuinely joining the AP and a laptop on the same home network (not just build/boot). Confirmed live: WiFi setup page loads with a real network scan list; saving credentials joins the home network (`wifiStatus: Connected`, real IP, confirmed via `/api/v1/state` and a live boot log); the DNS captive-portal redirect works against real Android traffic (`generate_204` correctly gets a `302`); the full SPA dashboard (`/`) loads and renders correctly over the network, including live click-through of all four sub-pages (Configuration, Log, Firmware, WiFi); a real profile-switch round-trip via the dashboard; and a **full OTA upload cycle** - `curl`-uploaded the current build to a live device, confirmed `OTA: success, 1411840 bytes` in the log, a clean software reboot (not a crash) booting from the other OTA partition, and reconnection to WiFi, all after fixing the watchdog-crash bug above. **Still not verified:** SSE update cadence during an actual motor run (blocked on Phase 4 hardware) and `webLog()` coverage of motion/safety events (see gap noted below).
- **Gap found (not yet fixed):** `main/motion.cpp`'s safety-relevant state machine (jam detection, calibration, homing) never calls `webLog()` - only `showJamScreen()` (on-device LVGL) and `ESP_LOGx` (serial). Jam/stall/calibration/homing events currently never reach the web Log page, only the physical serial console.
...
- [x] **OTA rollback:** `CONFIG_BOOTLOADER_APP_ROLLBACK_ENABLE=y` (Phase 1). Added the missing `esp_ota_mark_app_valid_cancel_rollback()` self-check call in `app_main.cpp`, gated on `esp_ota_get_state_partition()` returning `ESP_OTA_IMG_PENDING_VERIFY`. **The self-check-marks-valid half is now verified end-to-end**: after a real OTA upload (see Phase 5), the post-boot log shows `OTA: image marked valid (rollback canceled)`, confirming the image really was `PENDING_VERIFY` and the check fired correctly - this line correctly never appeared on any of the many direct-`idf.py flash` boots earlier in this session. **Still not verified:** the actual rollback path (a bad OTA image that never calls the self-check reverting to the previous slot on next boot) - would need a deliberately-broken OTA image to test.
