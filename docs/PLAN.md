...
- [x] **#24:** decided — next release is **v2.0.0** (tags are always full three-part semver,
  never the `vX.Y` shorthand — ESP-IDF's own releases mix both, this project holds itself to the
  stricter convention). `CHANGELOG.md`'s `[Unreleased]` section already covers this migration in
  full; it gets renamed to `## [2.0.0] - <date>` at actual release time per Keep a Changelog
  convention, once the release tag is cut. **Superseded resolution:** this was first done by
  bumping a hand-maintained `#define FW_VERSION "2.0"` in `main/config.h`; that macro has since
  been removed entirely in favour of the version ESP-IDF already derives from git —
  `esp_app_desc_t.version`, auto-populated at build time from `git describe --always --tags
  --dirty` (no `version.txt`, no `VERSION` in `project()`, no `PROJECT_VER`). The firmware reads
  it via `esp_app_get_description()->version` in the boot banner, in `buildStateJSON()`
  (`/api/v1/state`) and in `/api/v1/info`, so all three report the identical string. Verified on
  hardware: `"v2.0.0"` with a `v2.0.0` tag at HEAD, a commit hash (plus `-dirty`) otherwise —
  also caught and fixed a real bug here: the dashboard used to prepend its own `"v"` to whatever
  it was shown, which would have rendered a tagged release as `"vv2.0.0"` (`main/net/index_html.h`
  now renders the git-derived string verbatim instead). Cutting v2.0.0 is now just
  `git tag v2.0.0 && git push --tags` + a GitHub Release — no source file to bump.
